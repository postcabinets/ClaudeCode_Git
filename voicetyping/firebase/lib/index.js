...
function checkRateLimit(deviceId) {
    ...
    if (!entry || now > entry.resetAt) {
        rateLimitMap.set(deviceId, { count: 1, resetAt: now + 60000 });
        return true;
    }
    if (entry.count >= RATE_LIMIT_PER_DEVICE)
        return false;
    entry.count++;
    return true;
}
...
exports.formatText = (0, https_1.onRequest)({ cors: true, region: "asia-northeast1" }, async (req, res) => {
    if (req.method !== "POST") {
